...
namespace CRM_System.DAL
{
    public partial class EFContextFactory
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static CRM_SystemEntities GetCurrentDbContext()
        {
            //当第二次执行的时候直接取出线程嘈里面的对象
            //CallContext:是线程内部唯一的独用的数据槽(一块内存空间)
            //数据存储在线程栈中
            //线程内共享一个单例
            CRM_SystemEntities dbcontext = CallContext.GetData("DbContext") as CRM_SystemEntities;

            //判断线程里面是否有数据
            if (dbcontext == null)  //线程的数据槽里面没有次上下文
            {
                dbcontext = new CRM_SystemEntities();  //创建了一个EF上下文
                //存储指定对象
