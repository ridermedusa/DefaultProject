...
function BindDel() {
    $(document).on("click", "[name='roledel']", function () {
        ...
        parent.layer.confirm("您是否要删除这条数据", function () {
            //开始删除
            $.Majax("/Admin/BasicParams/DelRow", { "RowID": rid }, function (data) {
                if (data.Code == "0") {
                    parent.layer.close();
...
function BindEdit() {
    ...
    $(document).on("click", "[name='roleedit']", function () {
        ...
        $.OpenParentEdit("/Admin/BasicParams/Edit/" + roleedit, 570, 850, "Edit");
    });
    $(document).on("click", "[name='roleadd']", function () {
        $.OpenParentEdit("/Admin/BasicParams/Edit", 570, 850, "Edit");
    });
}