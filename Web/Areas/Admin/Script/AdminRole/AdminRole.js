/// <reference path="../../Views/layer/Admin/AdminPage.aspx" />
$(function () {
...
function GetBodyinfo(pageindex) {
    ...
    $.post("/Admin/AdminRole/GetTableinfo", { "pageindex": pageindex, "serchJson": JSON.stringify(Jsondate) }, function (data) {
        layer.close(loginload);
        var pageContDom = "";
        var Mi = 1;
        for (var i = 0; i < data.length; i++) {
            pageContDom += " <tr>"; 
            pageContDom += "          <td title=\"" + data[i].ROLENAME + "\">" + data[i].ROLENAME + "</td>";
            pageContDom += "         <td>";
...
function BindDel() {
    $(document).on("click", "[name='roledel']", function () {
        ...
        layer.confirm("您是否要删除这条数据", function () {
            //开始删除
            $.Majax("/Admin/AdminRole/DelRow", { "RowID": rid }, function (data) {
                if (data.Code == "0") {
                    layer.close();
...
function BindEdit() {
    $(document).on("click", "[name='roleedit']", function () {
        ...
        $.OpenParentEdit("/Admin/AdminRole/AdminRole/" + roleedit, 570, 850, "Edit");
    });
    $(document).on("click", "[name='roleadd']", function () {
        $.OpenParentEdit("/Admin/AdminRole/AdminRole", 570, 850, "Edit");
    });
}
